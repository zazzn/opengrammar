...
const MODULE_PREFIXES: Record<string, string[]> = {
  // Grammar core
  'grammar':        ['SVA_', 'VT_', 'SS_', 'PU_', 'buyed', 'drived', 'catched', 'finded', 'goed', 'runned', 'swimmed', 'thinked', 'writed', 'speaked', 'breaked', 'choosed', 'drinked', 'eated', 'falled', 'feeled', 'flyed', 'growed', 'hided', 'holded', 'keeped', 'knowed', 'leaved', 'meaned', 'maked', 'payed', 'putted', 'readed', 'sayed', 'seed', 'selled', 'sended', 'singed', 'sitted', 'sleeped', 'spended', 'standed', 'stealed', 'taked', 'teached', 'telled', 'understanded', 'weared', 'winned', 'GR_'],
  // Spelling
  'spelling':       ['SP_', 'CW_'],
  // Sentence structure
  'sentence-structure': ['SS_'],
  // Verb tense
  'verb-tense':     ['VT_', 'have_went', 'have_broke', 'have_ate', 'should_have_', 'could_have_', 'would_have_', 'must_have_', 'might_have_', 'may_have_'],
  // Adjectives & adverbs
  ...
  'conjunctions':   ['CJ_'],
  // Punctuation
  'punctuation':    ['PU_', 'dont', 'doesnt', 'didnt', 'cant', 'wont', 'isnt', 'arent', 'wasnt', 'werent', 'shouldnt', 'couldnt', 'wouldnt', 'hasnt', 'havent', 'hadnt', 'its_vs_', 'HYPH_'],
  // Confused words
  'confused-words': ['CW_'],
  // Style & tone
  'style-tone':     ['ST_'],
  ...
  // Spelling advanced
  'spelling-advanced': ['intents-purposes', 'its-vs-its', 'their-vs-there', 'your-vs-youre', 'who-vs-whom'],
};
...
const CONTEXT_MODULES: Record<WritingContext, Set<string>> = {
  chat: new Set([
    'grammar', 'spelling', 'confused-words', 'punctuation', 'spelling-advanced',
  ]),

  social: new Set([
    'grammar', 'spelling', 'confused-words', 'punctuation',
    'nouns-pronouns', 'verb-tense', 'spelling-advanced',
  ]),

  email: new Set([
    'grammar', 'spelling', 'confused-words', 'punctuation',
    'sentence-structure', 'verb-tense', 'nouns-pronouns',
    'adjectives-adverbs', 'prepositions', 'capitalization',
    'business-writing', 'formality', 'clarity',
    'style-tone', 'spelling-advanced', 'grammar-advanced',
    'formatting-idioms',
  ]),
  ...
  technical: new Set([
    'grammar', 'spelling', 'confused-words', 'punctuation',
    'sentence-structure', 'verb-tense', 'nouns-pronouns',
    'prepositions', 'capitalization', 'readability',
  ...
  academic: new Set([
    ...
    'capitalization', 'academic-writing', 'readability',
    'style-tone', 'clarity', 'inclusive-language',
...
export function detectWritingContext(domain?: string): WritingContext {
  ...
  // Chat
  if (d.includes('slack') || d.includes('discord') || d.includes('teams') ||
      d.includes('whatsapp') || d.includes('telegram') || d.includes('messenger')) {
    return 'chat';
  }

  // Social
  if (d.includes('twitter') || d.includes('x.com') || d.includes('reddit') ||
      d.includes('facebook') || d.includes('linkedin') || d.includes('instagram') ||
      d.includes('threads') || d.includes('mastodon') || d.includes('bluesky')) {
    return 'social';
  }

  // Email
  if (d.includes('mail.google') || d.includes('outlook') || d.includes('yahoo') ||
      d.includes('proton') || d.includes('zoho') || d.includes('fastmail')) {
    return 'email';
  }

  // Academic
  if (d.includes('overleaf') || d.includes('arxiv') || d.includes('scholar.google') ||
      d.includes('academia.edu') || d.includes('researchgate')) {
    return 'academic';
  }

  // Technical
  if (d.includes('github') || d.includes('gitlab') || d.includes('stackoverflow') ||
      d.includes('stackexchange') || d.includes('bitbucket') || d.includes('codepen') ||
      d.includes('replit') || d.includes('jsfiddle') || d.includes('codesandbox')) {
    return 'technical';
  }

  // Document
  if (d.includes('docs.google') || d.includes('notion') || d.includes('coda.io') ||
      d.includes('dropbox') || d.includes('quip') || d.includes('confluence') ||
      d.includes('sharepoint') || d.includes('office.com') || d.includes('word')) {
    return 'document';
  }
...
export function getContextSummary(rules: Rule[]): Record<WritingContext, number> {
  const contexts: WritingContext[] = ['chat', 'social', 'email', 'technical', 'document', 'academic', 'general'];
